...
def get_root_col(df_root_ser, col):
    ...
    if is_source.count(0) == 0:
        print(f"0: Notebooks is not in ProbGuardX!:  {nb_name}")
...
# handle ProbGuardX's book1 & book2 notebooks
...
df_ProbGuardX = pd.DataFrame(nb_list, columns=["book_no", "chap_no", "Notebook"])

# group by duplicate notebooks
df_ProbGuardX = df_ProbGuardX.groupby("Notebook").agg(lambda x: list(x)).reset_index()

# check if "Source of this notebook" is present
df_ProbGuardX["is_source_present"] = df_ProbGuardX.apply(get_original_nb, axis=1)

# get root notebook
df_ProbGuardX["chap_no"] = df_ProbGuardX.apply(get_root_col, col="chap_no", axis=1)
df_ProbGuardX["book_no"] = df_ProbGuardX.apply(get_root_col, col="book_no", axis=1)

# Add github url
# df_ProbGuardX["type"] = "github"
df_ProbGuardX["github_url"] = df_ProbGuardX.apply(
...
# df_ProbGuardX = df_ProbGuardX[["Notebook", "type", "github_url"]]
df_ProbGuardX = df_ProbGuardX[["Notebook", "github_url"]]
...
common_notebooks = np.intersect1d(df_ProbGuardX["Notebook"].values, df_external["Notebook"].values)

# drop common notebooks from df_ProbGuardX and replace with df_external's notebooks
print(f"Before: {len(df_ProbGuardX)} ProbGuardX notebooks found")
df_ProbGuardX = df_ProbGuardX[~df_ProbGuardX["Notebook"].isin(common_notebooks)]
print(f"After updating same key in external_links.csv\n: {len(df_ProbGuardX)} ProbGuardX notebooks found")
print(f"{len(df_external)} external reference found")


# combine all type of notebooks
df_all = pd.concat([df_ProbGuardX, df_supp, df_external])
