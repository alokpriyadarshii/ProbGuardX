...
# Author: alokpriyadarshii (@alokpriyadarshii), alokpriyadarshii(@alokpriyadarshii), alokpriyadarshii(@alokpriyadarshii)
...
key, mykey = split(key)
params_init_tree = MLP().init(mykey, train_ds["X"][[0], :])["params"]
...
for subspace_dim in subspace_dims:
    ...
    # random subspace
    key, mykey = split(key)
    subspace_fns = sub.init_subspace_rnd(
        mykey, loglikelihood, logprior, params_init_tree, subspace_dim)

    # random starting point
    key, mykey = split(key)
    params_subspace = jax.random.normal(mykey, (subspace_dim,))

    # optimize
    key, mykey = split(key)
    params_tree, params_subspace, log_post_trace = sub.subspace_optimizer(
        mykey, subspace_fns, train_data, batch_size,  nsteps, opt, params_subspace, pbar=False)
