...
notebooks_raw = subprocess.run(cmd, check=False, shell=True, capture_output=True, text=True)
if notebooks_raw.stderr:
    warnings.warn(notebooks_raw.stderr)
timestamped_notebooks = []
if notebooks_raw.returncode == 0 and notebooks_raw.stdout:
    ...
    for notebook in glob("notebooks/book*/*/*.ipynb"):
        if in_ignore_list(notebook):
            print(f"******** {notebook} skiped!! *********")
            continue
        timestamped_notebooks.append((int(os.path.getmtime(notebook)), notebook))
timestamped_notebooks.sort(reverse=True)  # execute newer notebooks first


if "ProbGuardX_GA_RUNNER_ID" in os.environ:
    # we are in execute_all_notebooks
    notebooks = [
        notebook
        for i, (_, notebook) in enumerate(timestamped_notebooks)
        if i % 20 == int(os.environ["ProbGuardX_GA_RUNNER_ID"])
    ]
else:
